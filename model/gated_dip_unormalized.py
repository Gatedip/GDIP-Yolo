...
class GatedDIP(torch.nn.Module):
    ...
    def __init__(self,
        ...
        Args:
            ...
            num_of_gates (int, optional): _description_. Defaults to 6.
        """
    ...
    def forward(self, x : torch.Tensor):
        ...
        sharpning_out = self.sharpning(x,latent_out,gate[:,2])
        fog_out = self.defog(x,latent_out,gate[:,3])
        contrast_out = self.contrast(x,latent_out,gate[:,4])
        tone_out = self.tone(x,latent_out,gate[:,5])
