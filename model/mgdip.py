...
class VisionEncoder(torch.nn.Module):
    def __init__(self,encoder_output_dim=256,base_channel=32):
        ...
        # conv_1
        self.conv_1 = torch.nn.Sequential(torch.nn.Conv2d(3,base_channel*(2**1),kernel_size = 3 , stride = 1),
                                        torch.nn.ReLU(True),
                                        torch.nn.Conv2d(base_channel*(2**1),base_channel*(2**1),kernel_size = 3 , stride = 1 ),
                                        torch.nn.ReLU(True))
        self.max_pool_1 = torch.nn.AvgPool2d((3,3),(2,2))
        self.adp_pool_1 = torch.nn.AdaptiveAvgPool2d((1,1))
        self.linear_proj_1 = torch.nn.Sequential(torch.nn.Linear(base_channel*(2**1),encoder_output_dim),
                                                torch.nn.ReLU(True))
        
        # conv_2
        self.conv_2 = torch.nn.Sequential(torch.nn.Conv2d(base_channel*(2**1),base_channel*(2**2),kernel_size = 3 , stride = 1),
                                        torch.nn.ReLU(True),
                                        torch.nn.Conv2d(base_channel*(2**2),base_channel*(2**2),kernel_size = 3 , stride = 1 ),
                                        torch.nn.ReLU(True))
        self.max_pool_2 = torch.nn.AvgPool2d((3,3),(2,2))
        self.adp_pool_2 = torch.nn.AdaptiveAvgPool2d((1,1))
        self.linear_proj_2 = torch.nn.Sequential(torch.nn.Linear(base_channel*(2**2),encoder_output_dim),
                                                torch.nn.ReLU(True))
        # conv_3
        self.conv_3 = torch.nn.Sequential(torch.nn.Conv2d(base_channel*(2**2),base_channel*(2**3),kernel_size = 3 , stride = 1),
                                        torch.nn.ReLU(True),
                                        torch.nn.Conv2d(base_channel*(2**3),base_channel*(2**3),kernel_size = 3 , stride = 1 ),
                                        torch.nn.ReLU(True))
        self.max_pool_3 = torch.nn.AvgPool2d((3,3),(2,2))
        self.adp_pool_3 = torch.nn.AdaptiveAvgPool2d((1,1))
        self.linear_proj_3 = torch.nn.Sequential(torch.nn.Linear(base_channel*(2**3),encoder_output_dim),
                                                torch.nn.ReLU(True))
        
        # conv_4
        self.conv_4 = torch.nn.Sequential(torch.nn.Conv2d(base_channel*(2**3),base_channel*(2**4),kernel_size = 3 , stride = 1),
                                        torch.nn.ReLU(True),
                                        torch.nn.Conv2d(base_channel*(2**4),base_channel*(2**4),kernel_size = 3 , stride = 1 ),
                                        torch.nn.ReLU(True))
        self.max_pool_4 = torch.nn.AvgPool2d((3,3),(2,2))
        self.adp_pool_4 = torch.nn.AdaptiveAvgPool2d((1,1))
        self.linear_proj_4 = torch.nn.Sequential(torch.nn.Linear(base_channel*(2**4),encoder_output_dim),
                                                torch.nn.ReLU(True))
        
        # conv_5
        self.conv_5 = torch.nn.Sequential(torch.nn.Conv2d(base_channel*(2**4),base_channel*(2**5),kernel_size = 3 , stride = 1),
                                        torch.nn.ReLU(True),
                                        torch.nn.Conv2d(base_channel*(2**5),base_channel*(2**5),kernel_size = 3 , stride = 1 ),
                                        torch.nn.ReLU(True))
        self.max_pool_5 = torch.nn.AvgPool2d((3,3),(2,2))
        self.adp_pool_5 = torch.nn.AdaptiveAvgPool2d((1,1))
        self.linear_proj_5 = torch.nn.Sequential(torch.nn.Linear(base_channel*(2**5),encoder_output_dim),
                                                torch.nn.ReLU(True))
        
        # conv_6
        self.conv_6 = torch.nn.Sequential(torch.nn.Conv2d(base_channel*(2**5),base_channel*(2**6),kernel_size = 3 , stride = 1),
                                        torch.nn.ReLU(True),
                                        torch.nn.Conv2d(base_channel*(2**6),base_channel*(2**6),kernel_size = 3 , stride = 1 ),
                                        torch.nn.ReLU(True))
        self.adp_pool_6 = torch.nn.AdaptiveAvgPool2d((1,1))
        self.linear_proj_6 = torch.nn.Sequential(torch.nn.Linear(base_channel*(2**6),encoder_output_dim),
                                                torch.nn.ReLU(True))
    def forward(self,x):
        out_x = self.conv_1(x)
